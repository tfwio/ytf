	...
	public class IniCollection : Dictionary<string,Dictionary<string,string>>
	{
		...
		internal void FromInstance(object input_o, string default_category = null)
		{
			...
			foreach (var i in WritableProperties)
			{
				...
				var result = input_o.GetStringValue(i.Name, string.Empty);
				this[group_name].Add(key_name, ali_attr.IsRoot ? result.Encode(): result);
			}
		}
		...
		public void ToInstance(object class_instance)
		{
			...
			foreach (var i in WritableProperties)
			{
				...
				if (has)
				{
					if (ali_attr.IsRoot)
					{
						if (!class_instance.SetInstanceValue(i.Name, this[group_name, key_name].Encode()))
							throw new Exception("Error writing instance value");
					}
					else if (!class_instance.SetInstanceValue(i.Name, this[group_name, key_name]))
						throw new Exception("Error writing instance value");
				}
			}
		}
		...
		public void Write(FileInfo file)
		{
			...
			using (var writer = new StreamWriter(file.FullName, false, System.Text.Encoding.UTF8))
			{
				foreach (var k in Keys)
				{
					...
					{
						writer.WriteLine("{0} = {1}", kv.Key, kv.Value.Encode());
						writer.WriteLine("");
					}
				}
			}
		...
		{
			get {
				return HasGroupAndKey(kGroup,kKey) ? this[kGroup][kKey] : null;
			}
