...
namespace YouTubeDownloadUtil
{
	...
	class ConfigModel
	{
		...
		[IniKey(Group = "global", Alias = "target", IsRoot = true)] public string TargetOutputDirectory { get; set; }
		...
		[IniKey(Group = "global", Alias = "AVConv_bin", IsRoot = true)] public string PathAVConv { get; set; }

		/// <summary>path containing FFmpeg.exe</summary>
		[IniKey(Group = "global", Alias = "FFmpeg_bin", IsRoot = true)] public string PathFFmpeg { get; set; }

		/// <summary>Path containing youtube-dl and atomicparsley.</summary>
		[IniKey(Group = "global", Alias = "youtube-dl_bin", IsRoot = true)] public string PathYoutubeDL { get; set; }
		...
		[Ignore] public List<string> DownloadTargetsList
		{
			get
			{
				var oldList = new List<string>(DownloadTargets.Split(';'));
				var newList = new List<string>();
				foreach (var item in oldList)
				{
					if (string.IsNullOrEmpty(item)) continue;
					var currentPath = item.Decode();
		...
		{
			var p = Path.GetFullPath(path.Decode());
		...
		{
			var tmpPath = Path.GetFullPath(path.Decode());
			if (!Directory.Exists(tmpPath)) return;
			var tg = new List<string>(DownloadTargetsList);
			if (tg.Contains(tmpPath)) return;
			if (!string.IsNullOrEmpty(tmpPath)) tg.Add(tmpPath);
